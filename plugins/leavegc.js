let handler = async (m, { conn, args, command }) => {
    ...
    if (command.endsWith('all') || command.endsWith('semua')) {
        for (let i = 0; i < chat.length; i++) { // For loops
            await m.reply('Bye👋, Bot will leave the group', chat[i].jid)
        ...
        if (!ada) throw 'Bot are not in that group!!'
        await m.reply('Byee👋, Bot will leave the group', args[0])
        await conn.groupLeave(args[0])
        await m.reply('Succes!!')
    } else {
        if (!m.isGroup) return global.dfail('group', m, conn)
        await m.reply('Bye👋, Bot will leave the group', m.chat) // WKWKW pesannya sama semua. gk kreatif :v
